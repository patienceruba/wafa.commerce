...
from users.schemas import UserCreate, UserUpdate, UserResponse, LoginRequest, VerifyOTPRequest, GoogleAuthRequest
from services.email_service import send_otp_email, send_welcome_email

logger = logging.getLogger(__name__)

# Password hashing using direct bcrypt to ensure Python 3.12+ compatibility
...
def get_password_hash(password: str) -> str:
    # Truncate to 72 bytes to adhere strictly to bcrypt limitations
...
def resolve_device_ip(ip: str = None) -> str:
    if ip and ip.strip() and ip.strip() not in ("127.0.0.1", "localhost", "::1"):
        clean_ip = ip.strip()
        if clean_ip.startswith("192.168.") or clean_ip.startswith("10.") or clean_ip.startswith("172."):
            return clean_ip
    try:
        import socket
...
class UserService:
    ...
    def create_user(self, user: UserCreate) -> UserResponse:
        try:
            ...
            db_user = self.db.query(User).filter(User.username.ilike(clean_username)).first()
            if db_user:
                raise ValueError("Username already exists")
            
            db_user = self.db.query(User).filter(User.email.ilike(clean_email)).first()
            if db_user:
    ...
    def authenticate_user(self, login_data: LoginRequest) -> str:
        ...
        logger.info(
            f"[OTP DEBUG] Generated OTP for user {user.username}: {otp_code} (session {session_id})"
        )
    ...
    def verify_otp(self, verify_data: VerifyOTPRequest, ip_address: str = None, user_agent: str = None) -> dict:
        otp_session = self.db.query(OTPSession).filter(OTPSession.session_id == verify_data.session_id).first()
        if not otp_session:
        ...
        access_token, refresh_token = self._generate_auth_tokens(user, ip_address=ip_address, user_agent=user_agent)
    ...
    def authenticate_google_user(self, google_data: GoogleAuthRequest, ip_address: str = None, user_agent: str = None) -> dict:
        ...
        try:
            ...
            if token.count('.') == 2:
                ...
                verify_url = f"https://www.googleapis.com/oauth2/v3/userinfo?access_token={token}"
                
            req = urllib.request.Request(verify_url, headers={"User-Agent": "PETS-Auth-Service/1.0"})
            with urllib.request.urlopen(req, timeout=8) as resp:
                if resp.status == 200:
                    verified_payload = json.loads(resp.read().decode("utf-8"))
        except Exception as e:
            logger.error(f"Google Token verification failed: {e}")
            raise ValueError("Invalid or expired Google authentication token. Verification failed.")
        ...
        if verified_payload.get("email_verified") is False or verified_payload.get("email_verified") == "false":
            raise ValueError("Google account email is not verified by Google.")
            
        email = str(verified_payload["email"]).strip().lower()
        f_name = verified_payload.get("given_name") or (verified_payload.get("name", "").split()[0] if verified_payload.get("name") else "Google")
        l_name = verified_payload.get("family_name") or (" ".join(verified_payload.get("name", "").split()[1:]) if verified_payload.get("name") and len(verified_payload.get("name", "").split()) > 1 else "User")
        ...
        if not user:
            ...
            if not user.is_active:
                raise ValueError("User account is inactive. Please contact system administrator.")
    ...
    def refresh_access_token(self, refresh_token_str: str) -> dict:
        # Find refresh token
        db_token = self.db.query(RefreshToken).filter(RefreshToken.token == refresh_token_str).first()
    ...
    def update_user(self, user_id: int, user_update: UserUpdate) -> User:
        ...
        if user_update.email is not None:
            existing = self.db.query(User).filter(User.email == user_update.email, User.id != user_id).first()
            if existing:
    ...
    def _generate_auth_tokens(self, user: User, ip_address: str = None, user_agent: str = None) -> tuple[str, str]:
    ...
    def get_admin_users_summary(self) -> list[dict]:
        ...
        for u in users:
            ...
            distinct_devices = len(set((t.ip_address, (t.user_agent or "")[:50]) for t in active_tokens))
            active_sessions = max(1 if active_tokens else 0, distinct_devices)

            # Accounts count & breakdown
            user_accounts = self.db.query(Account).filter(Account.user_id == u.id).all()
            ...
            txs = self.db.query(Transaction).filter(Transaction.user_id == u.id).all()
            tx_count = len(txs)
            
            spent_rwf = sum(t.amount for t in txs if (t.currency == 'RWF' or (not t.currency and t.amount > 50)) and t.type in ['expense', 'debit', 'payment'])
            income_rwf = sum(t.amount for t in txs if (t.currency == 'RWF' or (not t.currency and t.amount > 50)) and t.type == 'income')
            spent_usd = sum(t.amount for t in txs if t.currency == 'USD' and t.type in ['expense', 'debit', 'payment'])
            income_usd = sum(t.amount for t in txs if t.currency == 'USD' and t.type == 'income')
            ...
            result.append({
                ...
                "last_login": last_activity.login_time.isoformat() if last_activity and last_activity.login_time else None,
                "last_ip": resolve_device_ip(last_activity.ip_address if last_activity else None),
                "last_user_agent": last_activity.user_agent if last_activity else None,
    ...
    def get_admin_active_sessions(self) -> list[dict]:
        ...
        result = []
        for t in tokens:
            device_key = (t.user_id, t.ip_address or "127.0.0.1", (t.user_agent or "Web Browser")[:50])
            ...
            last_active_time = last_activity.login_time if last_activity and last_activity.login_time else t.created_at
            # Active in the last 15 minutes is live online
            is_online = bool(last_active_time and (now_naive - last_active_time).total_seconds() < 900)
            ...
            result.append({
                ...
                "is_online": is_online,
                "last_active": last_active_time.isoformat() if last_active_time else None,
    ...
    def revoke_session_by_id(self, session_id: int):
        token = self.db.query(RefreshToken).filter(RefreshToken.id == session_id).first()
        if not token:
    ...
    def change_user_role(self, user_id: int, new_role: str, requesting_admin_id: int) -> dict:
    ...
    def get_top_transactors_leaderboard(self) -> list[dict]:
        ...
        for u in users:
            ...
            tx_count = len(txs)
            total_spent = sum(t.amount for t in txs if t.type in ['expense', 'debit', 'payment'])
            total_income = sum(t.amount for t in txs if t.type == 'income')
            total_transfers = sum(t.amount for t in txs if t.type == 'transfer')
            latest_tx = max(txs, key=lambda x: x.transaction_date or datetime.min.date()) if txs else None
            ...
            ip_addr = (last_activity.ip_address if last_activity and last_activity.ip_address else None) or (last_token.ip_address if last_token else "127.0.0.1")
            ...
            board.append({
                ...
                "last_ip": ip_addr,
                "accounts_count": len(user_accounts),
                "accounts_list": [{"id": a.id, "name": a.name, "type": a.type, "currency": a.currency} for a in user_accounts],
                "transaction_count": tx_count,
                "total_spent": total_spent,
                "total_income": total_income,
                "total_transfers": total_transfers,
                "latest_transaction_date": str(latest_tx.transaction_date) if latest_tx and latest_tx.transaction_date else None,
    ...
    def get_user_transactions_for_admin(self, user_id: int) -> list[dict]:
        from transactions.models import Transaction
        from accounts.models import Account
        txs = self.db.query(Transaction).filter(Transaction.user_id == user_id).order_by(Transaction.transaction_date.desc(), Transaction.id.desc()).all()
        result = []
        for t in txs:
            da = self.db.query(Account).filter(Account.id == t.debit_account_id).first() if t.debit_account_id else None
            ca = self.db.query(Account).filter(Account.id == t.credit_account_id).first() if t.credit_account_id else None
            result.append({
                "id": t.id,
                "amount": t.amount,
    ...
    def admin_reset_user_password(self, user_id: int, new_password: str) -> dict:
        ...
        self.db.commit()
        return {"detail": f"Password for {user.username} has been reset and all active sessions revoked"}

    def revoke_all_sessions_for_user(self, user_id: int) -> dict:
        count = self.db.query(RefreshToken).filter(RefreshToken.user_id == user_id, RefreshToken.revoked == False).update({"revoked": True})
        self.db.commit()
